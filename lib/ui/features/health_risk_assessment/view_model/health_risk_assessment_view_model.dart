...
class PersonalRiskAssessmentViewModel extends ChangeNotifier {
  ...
  /// are in the danger (red) zone, OR when lifestyle / gender-specific risk
  /// factors are present.
  bool get isCaution =>
      !hasRedMetrics &&
      (systolicStatus == HealthMetricStatus.orange ||
          ...
          (showFemaleQuestions &&
              (papSmear == true ||
                  breastExam == true ||
                  (showMammogramQuestion && mammogram == true))) ||
          (showMaleQuestions &&
              (prostateCheck == true || prostateTested == true)));
  ...
  Future<void> submitResults({
    ...
    if (nurseVM != null) {
      if (nurseVM.nurseFirstNameController.text.isEmpty ||
        ...
        onValidationFailed
            ?.call('Please complete all nurse details (name, rank, SANC No).');
        return;
      }
    ...
    try {
      ...
      final hraScreening = HraScreening(
        ...
        notReferredReason:
            nurseVM?.notReferredReasonController.text.isEmpty == true
                ? null
