...
class EventReportExporter {
  ...
  void _buildEventDetailsSheet(
    ...
    _appendRow(
        sheet, 'Expected Participants', event.expectedParticipation.toString());
  ...
  void _buildScreeningStatsSheet(
    ...
    _appendStatRow(
        sheet, 'Underweight (BMI < 18.5)', hra.underweight, hra.total);
    _appendStatRow(
        sheet, 'Normal Weight (BMI 18.5–24.9)', hra.normal, hra.total);
    _appendStatRow(
        sheet, 'Overweight (BMI 25–29.9)', hra.overweight, hra.total);
    ...
    _appendStatRow(
        sheet, 'High Cholesterol', hra.highCholesterol, hra.total);
    ...
    _appendStatRow(
        sheet, 'PAP Smear Collected', cancer.papCollected, cancer.total);
    _appendStatRow(sheet, 'PAP Positive / Abnormal', cancer.papPositive,
        cancer.papCollected);
    _appendStatRow(sheet, 'PSA Done', cancer.psaDone, cancer.total);
    _appendStatRow(
        sheet, 'PSA Abnormal (> 4.0)', cancer.psaAbnormal, cancer.psaDone);
    ...
    _appendStatRow(
        sheet, 'First-time Testers', hct.firstTimeTesters, hct.total);
  ...
  CancerStats _computeCancerStats(List<CancerScreening> list) {
    ...
    for (final s in list) {
      ...
      if (papResult != null &&
          (papResult.contains('positive') ||
              papResult.contains('abnormal'))) {
        papPositive++;
      }
      ...
      if (s.breastLightExamFindings != null &&
          s.breastLightExamFindings!.isNotEmpty) symptomatic++;
