    ...
    try {
      ...
      try {
        ...
        if (querySnapshot.docs.isNotEmpty) {
          ...
          debugPrint(
              'Password reset request timestamp updated for user: $userId');
        } else {
          debugPrint(
              'Password reset: User not found in Firestore, but email sent via Firebase Auth');
        }
      } catch (firestoreError) {
        // Don't fail if Firestore update fails - the reset email was already sent
        debugPrint(
            'Warning: Failed to update Firestore timestamp: $firestoreError');
      }
  ...
  Future<List<UserModel>> getAllUsers() async {
    try {
      ...
      try {
        ...
        debugPrint('FirebaseAuthService: local user cache read failed: $localErr');
      }
      return [];
    }
  }
