...
class DataMigrationService {
  ...
  class MigrationResult {
    ...
    @override
    String toString() {
      return 'MigrationResult(users: $usersMigrated, events: $eventsMigrated, '
      ...
      for (final userEntity in localUsers) {
        try {
          ...
          if (existingDoc.exists) {
            debugPrint('User ${userEntity.email} already exists in Firestore, skipping');
            continue;
          }
  ...
  Future<int> migrateEvents() async {
    try {
      ...
      debugPrint('Starting event migration: ${localEvents.length} events found');
      ...
      for (final eventEntity in localEvents) {
        try {
          ...
          if (existingDoc.exists) {
            debugPrint('Event ${eventEntity.title} already exists in Firestore, skipping');
            continue;
          }
  ...
  Map<String, dynamic> _eventEntityToMap(EventEntity entity) {
    return {
      ...
      'actualStartTime':
          entity.actualStartTime != null ? Timestamp.fromDate(entity.actualStartTime!) : null,
      'actualEndTime':
          entity.actualEndTime != null ? Timestamp.fromDate(entity.actualEndTime!) : null,
