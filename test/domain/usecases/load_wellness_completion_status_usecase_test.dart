...
class MockCancerRepo extends Mock implements FirestoreCancerScreeningRepository {}
...
void main() {
  ...
  group('LoadWellnessCompletionStatusUseCase – with consent', () {
    test('consent flags reflect consented services', () async {
      final consent = _buildConsent(hra: true, hct: true, tb: false, cancer: false);
      when(() => mockConsent.getConsentsByMember(any()))
  ...
  group('LoadWellnessCompletionStatusUseCase – screening completion', () {
    test('all screenings marked completed when records match the event', () async {
      when(() => mockConsent.getConsentsByMember(any()))
          .thenAnswer((_) async => [_buildConsent(hra: true, hct: true, tb: true, cancer: true)]);
      when(() => mockHra.getHraScreeningsByMember(any()))
    ...
    test('screenings for other events do not set completion flags', () async {
      ...
      when(() => mockHra.getHraScreeningsByMember(any()))
          .thenAnswer((_) async => [_buildHraScreening(eventId: _otherEventId)]);
      when(() => mockHiv.getHivScreeningsByMember(any()))
          .thenAnswer((_) async => []);
      when(() => mockTb.getTbScreeningsByMember(any()))
          .thenAnswer((_) async => []);
  ...
  group('LoadWellnessCompletionStatusUseCase – resilience', () {
    test('returns partial result when a single repo throws (non-fatal)', () async {
      // Consent succeeds.
      when(() => mockConsent.getConsentsByMember(any()))
