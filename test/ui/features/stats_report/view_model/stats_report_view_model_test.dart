...
class MockMemberEventRepo extends Mock
...
void main() {
  ...
  group('StatsReportViewModel.computeStats – aggregation', () {
    ...
    test('computes participationRate correctly', () {
      final events = [
        _buildEvent(
            id: 'e-rate', expectedParticipation: 200, screenedCount: 100),
      ];
      final vm = _buildViewModel();
      final stats = vm.computeStats(events);
  ...
  group('StatsReportViewModel.applyFilters – past tab', () {
    test('past tab returns only completed events', () {
      final completed =
          _buildEvent(id: 'past-c', status: WellnessEventStatus.completed);
      final scheduled =
          _buildEvent(id: 'past-s', status: WellnessEventStatus.scheduled);
      final inProgress =
          _buildEvent(id: 'past-ip', status: WellnessEventStatus.inProgress);
      final vm = _buildViewModel();
  ...
  group('StatsReportViewModel.applyFilters – search', () {
    test('search filter narrows by title (case-insensitive)', () {
      final a = _buildEvent(
          id: 'a',
          title: 'Cape Town Health Day',
          status: WellnessEventStatus.completed);
      final b = _buildEvent(
          id: 'b',
          title: 'Johannesburg Wellness',
          status: WellnessEventStatus.completed);
      final vm = _buildViewModel();
  ...
  group('StatsReportViewModel.applyFilters – province filter', () {
    test('province filter narrows events to matching province', () {
      final gp = _buildEvent(
          id: 'gp', province: 'Gauteng', status: WellnessEventStatus.completed);
    ...
    test('"All" province filter returns all events', () {
      ...
      final result =
          vm.applyFilters(events, isLiveTab: false, provinceFilter: 'All');
